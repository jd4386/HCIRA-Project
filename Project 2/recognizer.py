...
# Function to compare the user's unistroke to the template unistrokes and return the best match
def recognize(currentUnistroke, unistrokes, returnNBest=False, preProcessUnistrokes=True):
    global HalfDiagonal, AngleRange, AnglePrecision

    if preProcessUnistrokes:
        try:
            # Preprocessing the user's unistroke
            preProcessedCurrentUnistroke = preprocess(currentUnistroke)
        except ZeroDivisionError:
            # If the user's unistroke is a straight line, return 'No match.'
            return 'No match.', '0'
    else:
        preProcessedCurrentUnistroke = currentUnistroke
    ...
    for key in unistrokes:
        ...
        d = distanceAtBestAngle(preProcessedCurrentUnistroke, unistrokes[key], -AngleRange, +AngleRange, AnglePrecision)

        # Adding the unistroke and the distance to the nBest dictionary
        nBest[key] = str(round((1.0 - d / HalfDiagonal), 2))
    ...
    # Returning the nBest dictionary if returnNBest is True else returning the best unistroke and the best distance
    return nBest if returnNBest else ([u.replace('_', ' '), str(round((1.0 - b / HalfDiagonal), 2))] if u else ['No match.', '0'])
...
def distanceAtBestAngle(points, T, a, b, threshold):
    ...
    # Loop until the difference between the angles is less than the threshold
...
# Function to return the distance between rotated points and the template
def distanceAtAngle(points, T, radians):
    # Rotate the points by the given angle
    newpoints = rotateBy(points, radians)
    # Return the distance between the rotated points and the template
...
# Function to return the distance between the points
def pathDistance(pts1, pts2):
    d = 0
    
    # Loop through the points and add the distance between them
...
# Function to apply the 4 steps of the algorithm to the points
def preprocess(points):
    global NumPoints, SquareSize, Origin

    # Resample the points
    preProcessedPoints = resample(points, NumPoints)
    
    # Rotate the points
    radians = indicativeAngle(preProcessedPoints)
    preProcessedPoints = rotateBy(preProcessedPoints, -radians)

    # Scale the points
    preProcessedPoints = scaleTo(preProcessedPoints, SquareSize)

    # Translate the points
    preProcessedPoints = translateTo(preProcessedPoints, Origin)
...
def resample(points, n):
    ...
    while i < len(points):
        ...
        if D + d >= I:
            # Get the new point between the current point and the previous point
            delta_distance = float((I - D) / d)

            qx = points[i - 1][0] + delta_distance * (points[i][0] - points[i - 1][0])
            qy = points[i - 1][1] + delta_distance * (points[i][1] - points[i - 1][1])
