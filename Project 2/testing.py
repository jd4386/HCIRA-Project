...
def readDataset(path, gestures):
    ...
    for U in range(1, 7):
        for gesture in gestures:
            for E in range(1, 11):
                key = f'{U:02d}/{gesture}/{E:02d}'

                # Reading the XML file
                filepath = os.path.join(path, f'User{U:02d}', f'{gesture}{E:02d}.xml')
        ...
        for E in examples:
            ...
            for iteration in range(iterations):
                ...
                for gesture in Gestures:
                    ...
                    train_examples_keys = [f'{U:02d}/{gesture}/{example:02d}' for example in train_examples]
                    test_example_key = f'{U:02d}/{gesture}/{test_example:02d}'
                ...
                for test_gesture_key, test_gesture_value in test_dataset.items():
                    ...
                    if test_gesture_key.split('/')[1] != sortedNBest[0][0].split('/')[1]:
                        recognition_flag = 0

                    # Results XML file row to be appended
                    result_row = [
                        f'User{U:02d}',
                        test_gesture_key.split('/')[1],
                        iteration + 1,
                        E,
                        len(train_dataset),
                        list(train_dataset.keys()),
                        test_gesture_key,
                        sortedNBest[0][0].split('/')[1],
                        recognition_flag,
                        sortedNBest[0][1],
                        sortedNBest[0][0],
                        top50NBest
                    ...
                    accuracy_key = f'User({U:02d}) - Gesture({test_gesture_key.split("/")[1]}) - No. of training examples({E})'
